...
def process(dbName, dbDir, dbExtens, fieldName, outDir, pixelmap_dir, ebvofMW=-1., nproc=8, mode='batch', snTypes=['faintSN', 'allSN']):
    ...
    if mode == 'batch':
        batch(dbName, dbDir, dbExtens, fieldName, outDir,
              pixelmap_dir, ebvofMW, snTypes, nproc)
    else:
        config = config_rec()
        confNames = snTypes
        for cf in confNames:
            ...
            for ibatch, vv in enumerate(spl):
                for val in vv:
                    cmd_ = cmd(dbName, dbDir, dbExtens, fieldName,
                               val, outDir, pixelmap_dir, 0, 0, nproc)
                    print(cmd_)
                    os.system(cmd_)
...
def batch(dbName, dbDir, dbExtens, fieldName, outDir, pixelmap_dir, ebvofMW, snTypes, nproc=8):
    ...
    for cf in confNames:
        ...
        for ibatch, vv in enumerate(spl):
            batch_indiv(dbName, dbDir, dbExtens, fieldName, outDir,
                        pixelmap_dir, ebvofMW, cf, vv, ibatch, nproc)
...
def batch_indiv(dbName, dbDir, dbExtens, fieldName, outDir, pixelmap_dir, ebvofMW, confname, config, ibatch, nproc=8):
    ...
    dirScript, name_id, log, cwd = prepareOut(
        dbName, fieldName, confname, ibatch)
    # qsub command
    ...
    for iconf, val in enumerate(config):
        cmd_ = cmd(dbName, dbDir, dbExtens, fieldName, val, outDir,
                   pixelmap_dir, ebvofMW, ibatch, iconf, nproc)
        script.write(cmd_+" \n")
...
def cmd(dbName, dbDir, dbExtens, fieldName, config, outDir, pixelmap_dir, ebvofMW, ibatch, iconfig, nproc):
    ...
    cmd += ' --ProductionIDSimu DD_{}_{}_error_model_{}_{}_{}'.format(
        dbName, fieldName, config['confName'], ibatch, iconfig)
...
parser.add_option('--dbDir', type='str',
                  default='/sps/lsst/cadence/LSST_SN_PhG/cadence_db/fbs_1.5/npy',
                  help='db dir [%default]')
...
# parser.add_option('--pixelmap_dir', type='str',
#                  default='/sps/lsst/users/gris/ObsPixelized_128', help='pixelmap directory [%default]')
...
for fieldName in fieldNames:
    process(opts.dbName, opts.dbDir, opts.dbExtens, fieldName, opts.outDir,
            opts.pixelmap_dir, opts.ebvofMW, opts.nproc, opts.mode, snTypes)
"""
process_new(vars(opts))
