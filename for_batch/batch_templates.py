...
def batch(x1,color,nproc=8,zmax=1.2):
    ...
    qsub = 'qsub -P P_lsst -l sps=1,ct=2:00:00,h_vmem=16G -j y -o {} -pe multicores {} <<EOF'.format(log,nproc)
    ...
    cmd = 'python run_scripts/templates/run_simulation_template.py --simul 1 --lcdiff 0 --x1 {} --color {} --nproc {} --zmax {}'.format(x1,color,nproc,zmax)
    script.write(cmd+" \n")
    cmd = 'python run_scripts/templates/run_simulation_template.py --simul 0 --lcdiff 1 --x1 {} --color {}'.format(x1,color)
    script.write(cmd+" \n")
    script.write("EOF" + "\n")
    script.close()
