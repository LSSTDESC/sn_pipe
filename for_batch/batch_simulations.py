...
def batch(dbDir,dbName,scriptref,nproc,season,diffflux,outDir):
    ...
    qsub = 'qsub -P P_lsst -l sps=1,ct=10:00:00,h_vmem=16G -j y -o {} -pe multicores {} <<EOF'.format(log,nproc)
    ...
    cmd='python {} --dbDir {} --dbName {} --nproc {} --season {} --diffflux {} --outDir {}'.format(scriptref,dbDir,dbName,nproc,season,diffflux,outDir)
    script.write(cmd+" \n")
    script.write("EOF" + "\n")
    script.close()
...
def batch_family(dbDir,familyName,arrayDb,scriptref,nproc,diffflux,outDir,x1,color,zmin,zmax):
    ...
    for dbName in arrayDb['dbName']:
        for season in range(1,11):
            cmd='python {} --dbDir {} --dbName {} --nproc {} --season {} --diffflux {} --outDir {}'.format(scriptref,dbDir,dbName,1,season,diffflux,'{}/{}'.format(outDir,familyName))
            cmd += ' --x1 {} --color {} --zmin {} --zmax {}'.format(x1,color,zmin,zmax)
            script.write(cmd+" \n")
    script.write("EOF" + "\n")
    script.close()
    os.system("sh "+scriptName)
...
for family in np.unique(toprocess['family']):
    ...
    sel = toprocess[idx]
    batch_family(dbDir,family,sel,'run_scripts/run_simulation_fromnpy.py',8,diffflux,outDir,x1,color,zmin,zmax)
