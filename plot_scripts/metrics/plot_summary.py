import numpy as np
import sn_plotter_metrics.sn_cadencePlotters as sn_plot
...
class Summary:
    def __init__(self, x1=-2.0, color=0.2, namesRef=['SNCosmo'],
       ...
       mag_range: list(float), opt
        ...
        for name in namesRef:
            self.Li_files = [
                '{}/Li_{}_{}_{}.npy'.format(refDir, name, x1, color)]
            self.mag_to_flux_files = [
                '{}/Mag_to_Flux_{}.npy'.format(refDir, name)]
    ...
    def loadFile(self, dirFile, dbName, fieldtype, metricName, nside=64):
        ...
        fileName = '{}/{}/{}_{}*.npy'.format(dirFile,
                                             dbName, dbName, metricName)
    ...
    def getMetricValues(self, dirFile, dbName, bands, m5_str='m5_median'):
        ...
        for band in bands:
            metricValuesSNR = self.loadFile(
                dirFile, dbName, '', 'SNR{}'.format(band))
        ...
        for band in bands:
            ...
            for name in self.namesRef:
                ...
                med_detect = np.median(
                    metricValuesSNR['frac_obs_{}'.format(name)])
...
parser.add_option("--dirFile", type="str", default='/sps/lsst/users/gris/MetricSummary',
                  help="metric file directory [%default]")
parser.add_option("--configFile", type="str", default='plot_scripts/cadenceCustomize_fbs14.csv',
                  help="config file (for plots) directory [%default]")
...
if not os.path.isfile('Summary.npy'):
    ...
    medValues = np.array(medList, dtype=[
                         ('band', 'U1'), ('dbName', 'U39'), ('zlim', 'f8'), ('detect_rate', 'f8')])
...
with open('opsim_runs.csv', newline='') as csvfile:
    ...
    for row in reader:
        ...
        idb = sel['dbName'] == name
        selb = sel[idb]
        print(row['num'], row['name'], row['group'], np.round(
            selb['zlim'][0], 4), np.round(selb['detect_rate'][0], 4))
        zlim = np.round(selb['zlim'][0], 4)
        detect_rate = np.round(selb['detect_rate'][0], 4)
        rowb = [row['num'], row['name'],
                row['group'], str(zlim), str(detect_rate)]
        rows.append(rowb)
