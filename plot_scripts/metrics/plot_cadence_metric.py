...
for season in [1, 2]:
    ...
    sn_plot.plotMollview(64, ssel, 'cadence_mean', 'cadence',
                         'days', 1., 30., band, dbName, saveFig=False)
...
for season in np.unique(metricValues['season']):
    ...
    print(np.min(ssel['cadence_mean']), np.max(ssel['cadence_mean']), season, len(
        ssel), len(np.unique(ssel['healpixID'])))
