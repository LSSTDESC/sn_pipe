...
def makeYaml(input_file,dbDir,dbName,nside,nproc,num):
...
def loop(healpixels,obsFocalPlane,dbDir,dbName,nside,x0_tab,j=0, output_q=None):

    config = makeYaml('input/param_simulation_gen.yaml',dbDir,dbName,nside,1,j)
...
config = makeYaml('input/param_simulation_gen.yaml',dbDir,dbName,nside,1,-1)
...
#for j in range(5,6):
    ida = tabpix[j]
    idb = tabpix[j+1]
    p=multiprocessing.Process(name='Subprocess-'+str(j),target=loop,args=(healpixels[ida:idb],obsFocalPlane,dbDir,dbName,nside,x0_tab,j,result_queue))
    p.start()
    break
