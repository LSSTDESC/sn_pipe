...
from sn_rubin_scheduler.ddf_presched import generate_ddf_scheduled_obs_auto
import pandas as pd
...
def get_val_field(df, field, colName):
    ...
    return df[df['field'] == field][colName].to_list()


def get_survey(survey_field_config, survey_field_visits):
    """
...
parser.add_option('--outputDir', type=str,
                  default='../desc_ddf_deep_rolling_auto',
...
df_survey = get_survey(f'{inputDir}/{ddf_survey_fields}',
...
ddf_kwargs = ddf_config(survey=df_survey)

print(ddf_kwargs)

# generate observations

observations = generate_ddf_scheduled_obs_auto(dist_tol=1,
