...
def mbcovCalc(covmb,names,vals):
    ...
    params=dict(zip(['x0','x1','c'],[vals[names.index('x0_fit')], vals[names.index('x1_fit')],vals[names.index('color_fit')]] ))

    resu = covmb.mbCovar(params,cov,['x0','x1','c'])

    sigmu_sq=resu['Cov_mbmb']
    sigmu_sq+=alpha**2*vals[names.index('Cov_x1x1')]+beta**2*vals[names.index('Cov_colorcolor')]
...
def multiproc(simu_name='', lc_name='',fit=None,covmb=None,nproc=1):
    ...
    for i in range(len(batch)-1):
        ...
        """
        p=multiprocessing.Process(name='Subprocess-'+str(i),target=procelem,args=(lc_name,simu[ida:idb],fit,covmb,i,result_queue))
        p.start()
        print('start',i)
...
class Fit_All:
    ...
    def dump(self,names,val,inum):
        ...
        tab['fitstatus'] = tab['fitstatus'].astype(h5py.special_dtype(vlen=str))
        tab.write(self.fit_out, 'fit_lc_{}'.format(inum), append=True,compression=True)
...
def run(dirFiles, prodid,outDir,nproc, covmb=None):
    ...
    multiproc(simu_name=simu_name,lc_name=lc_name,fit=fit,covmb=covmb,nproc=config['Multiprocessing']['nproc'])
