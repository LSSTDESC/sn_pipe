...
class SimuLC:
    """
    class to simulate LC from fakes
    ...
    zmax: float
      max redshift
    nproc: int
      number of proc for the simulation
    input_orig: str
      ???
    cad_orig: str
      ???
    ...
    def __init__(self, x1, color, zmin, zmax, nproc, input_orig, cad_orig, outdir):
        ...
        for i in range(len(batch)-1):
            ...
            p = multiprocessing.Process(name='Subprocess-'+str(i), target=self.zLoop, args=(
                zrange[ida:idb], 200+ida))
            p.start()
            print('start', i)

        print('end simu', time.time()-timeRef)

    def zLoop(self, zval, inum):
    ...
    def run(self, zval, ilc):
        ...
        cad = 0.1*(1.+zval)
        #cad = 0.5*(1.+zval)
        prodid = 'Fake_{}_{}_{}'.format(zval, self.x1, self.color)
        fake_name = 'input_prod/Fake_cadence_{}_{}_{}.yaml'.format(
            self.x1, self.color, zval)
        input_fakes = 'input_prod/input_fakes_{}_{}_{}.yaml'.format(
            self.x1, self.color, zval)
        self.make_input(zval, prodid,
                        fake_name, input_fakes)
        self.make_fake(duration, mjd_min, cad, fake_name)

        #cmd = 'python run_scripts/simulation/run_simulation.py '+input_fakes
        # os.system(cmd)

    def make_input(self, z, prodid, fakename, output_file):

        with open(self.input_orig, 'r') as file:
            filedata = file.read()
            ...
            filedata = filedata.replace('outdirname', self.outdir)

        with open(output_file, 'w') as file:
            file.write(filedata)

    def make_fake(self, duration, mjd_min, cad, output_file):
        with open(self.cad_orig, 'r') as file:
            filedata = file.read()
...
def Run(input_orig, cad_orig, zval, x1, color, outdir, ilc):
    ...
    input_fakes = 'input_prod/input_fakes_{}_{}_{}.yaml'.format(
        x1, color, zval)
    Replace_input(input_orig, x1, color, zval, prodid,
                  fake_name, outdir, input_fakes)
...
def simuLC(x1, color, nproc, input_orig, cad_orig, outdir, zmin, zmax):
    ...
    for i in range(len(batch)-1):
        ida = batch[i]
        idb = batch[i+1]
        p = multiprocessing.Process(name='Subprocess-'+str(i), target=zLoop, args=(
            input_orig, cad_orig, zrange[ida:idb], x1, color, outdir, 200+ida, i, result_queue))
        p.start()
        print('start', i)
...
parser.add_option("--simul", type="int", default=1,
                  help="perform simulation [%default]")
parser.add_option("--lcdiff", type="int", default=0,
                  help="produce LC with flux derivatives[%default]")
...
parser.add_option("--outDir", type="str", default='Templates_new',
                  help="output directory for [%default]")
parser.add_option("--outDirFinal", type="str",
...
if simul:
    mysim = SimuLC(x1, color, zmin, zmax, nproc, input_orig, cad_orig, outDir)
